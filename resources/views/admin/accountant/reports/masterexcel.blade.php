...
<body>
    ...
    @foreach ($companyName as $index => $company)
    <table>
        <tr>
            <th colspan="16" style="text-align: center; font-weight: bolder; font-size: 30px;">{{ $company }}</th>
        </tr>
        <tr>
            <th colspan="16" style="text-align: center;"><b>GST No.:</b>{{ $gstNumber[$index] }}</th>
        </tr>
        <tr>
            <th colspan="16" style="text-align: center;"><b>PAN No.:</b>{{ $panNumber[$index] }}</th>
        </tr>
        <tr>
            <th colspan="16" style="text-align: center;"><b>State:</b> {{ $state[$index] }}</th>
        </tr>
    </table>
...
<tbody style="border: 8px solid #000">
    @foreach ($summaries as $summary)
        @if ($summary->company_id == $comany_id[$index])
            @php
                $serviceCodeSummary = [];
                ...
                $grandPriceTotal += $summary->price_total;
                $grandGstAmount += $summary->gst_amount;
            @endphp
    ...
    {{-- Grand Total Row --}}
    <tr style="border: 2px solid #000; font-weight: bold; background: #f0f0f0">
        <td colspan="14" style="text-align: right; border: 1px solid #000;">Grand Total:</td>
        <td style="text-align: center; border: 1px solid #000;">{{ number_format($grandPriceTotal, 2) }}</td>
        <td style="text-align: center; border: 1px solid #000;">{{ number_format($grandGstAmount, 2) }}</td>
    </tr>
</tbody>
