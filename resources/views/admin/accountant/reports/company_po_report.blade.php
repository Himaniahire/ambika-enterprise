        ...
        <div class="container-xl px-4 mt-4">
            <div class="row">
                <div class="col-xl-12">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">Company/PO Report</div>
                        <div class="card-body">
...
$(document).ready(function() {
    ...
    $('#report-form').submit(function(e) {
        ...
        var month = $('#month').val();
        var poId = $('#po_id').val();
        var serviceCodeId = $('#service_code_id').val();

        $.ajax({
            url: '{{ route("fetch.data.company") }}',
            method: 'POST',
            data: {
            ...
            success: function(response) {
                if (response.summaries) {
                    ...
                    var dynamicTitle = response.dynamic_title;
                    var totalPrice = response.total_price;
                    var serviceCode = response.service_code || 'N/A';

                    var columns = [
                        { data: 'companyname', title: 'Company Name', defaultContent: 'N/A' },
                        { data: 'sum_no', title: 'Sum No', defaultContent: 'N/A' },
                        { data: 'invoice_date', title: 'Invoice Date', defaultContent: 'N/A' },
                        { data: 'invoice_no', title: 'Invoice No', defaultContent: 'N/A' },
                        { data: 'po_no', title: 'PO No', defaultContent: 'N/A' },
                        { data: 'price_total', title: 'Amount', defaultContent: 'N/A' },
                        { data: 'gst_amount', title: 'GST Amount', defaultContent: 'N/A' }
                    ];

                    if (poId && serviceCodeId) {
                        columns.push({ data: 'po_no', title: 'PO No', defaultContent: 'N/A' });
                        columns.push({ data: 'service_code', title: 'Service Code', defaultContent: 'N/A' });
                    } else {
                        columns.push({ data: 'total', title: 'Total Qty', defaultContent: 'N/A' });
                    }
                    ...
                    columns.push({
                        ...
                        render: function(data, type, row) {
                            return `<ul class="list-unstyled hstack gap-1 mb-0">
                                <li>
                                    <a class="btn btn-sm btn-soft-danger" target="_blank" href="/admin/accountant/summary/edit/${row.id}">
                    ...
                    dataTable = $('#reportTable').DataTable({
                        ...
                        buttons: [
                        {
                            ...
                            customize: function(xlsx) {
                                ...
                                // Function to calculate the grand total of gst_amount
                                function calculatetotalQty(rows) {
                                    return rows.reduce(function(total, row) {
                                        return total + parseFloat(row.total || 0); // Sum gst_amount values
                                    }, 0).toFixed(2); // Keep 2 decimal points
                                }
                                ...
                                var totalQty = calculatetotalQty(allRows);
                        ...
                        drawCallback: function(settings) {
                            ...
                            var grandTotal = data.reduce(function(acc, curr) {
                                return acc + parseFloat(curr.price_total);
                            }, 0).toFixed(2);
                            ...
                            var totalQty = data.reduce(function(acc, curr) {
                                return acc + parseFloat(curr.total);
                            }, 0).toFixed(2);

                            $('#reportTable tfoot').remove();

                            var grandTotalRow = '<tfoot><tr><td></td><td></td><td></td><td></td><td>Grand Total:</td><td>' + grandTotal + '</td><td>' + grandGSTTotal + '</td><td>' + totalQty + '</td></tr></tfoot>';
                    ...
                    // Ensure you have the element with id `rowCountDisplay` in your HTML
                    // Example: <div id="rowCountDisplay"></div>
                    ...
                    function calculatetotalQty(dataTable) {
                        var data = dataTable.rows({ page: 'current' }).data().toArray();
                        return data.reduce(function(acc, curr) {
                            return acc + parseFloat(curr.total);
                        }, 0).toFixed(2);
